...
3. Generates a single `fix-permissions` service that, per fixed path, runs `chown -R <uid>[:<gid>]` (or `chgrp`+`chmod g+rwX` when only `fsGroup` applies), plus `find <path> -type d -exec chmod g+s` whenever a gid is involved — same as K8s's own volume ownership walk
